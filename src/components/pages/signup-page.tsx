...
export function SignupPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (result.success) {
      toast({
        ...
        description: 'Welcome to Readium. Start writing your first story.',
      })
      navigate('home')
    } else {
      toast({
  ...
  return (
    <div className="min-h-[calc(100vh-8rem)] flex items-center justify-center px-4 py-12 gradient-hero relative overflow-hidden">
      ...
      <div className="absolute top-20 left-[10%] w-64 h-64 bg-primary/[0.03] rounded-full blur-3xl float-animation breathe pointer-events-none" />
      <div className="absolute bottom-20 right-[15%] w-48 h-48 bg-amber/[0.04] rounded-full blur-2xl float-slow breathe pointer-events-none" />
      ...
      >
        ...
        <motion.div variants={staggerChild} className="text-center mb-10">
          ...
          <p className="text-muted-foreground/60 text-sm mt-1.5 italic">
            Where every voice finds its reader
          </p>
        </motion.div>
        ...
        >
                  ...
                  <div
                    ...
                    className={`h-1 flex-1 rounded-full transition-colors duration-300 ${
                      password.length >= level * 3
                        ? level <= 1 ? 'bg-red-400' : level <= 2 ? 'bg-amber' : level <= 3 ? 'bg-amber' : 'bg-green-500'
                        : 'bg-muted'
                    }`}
                  />
                ))}
          ...
          <motion.div variants={staggerChild} className="flex items-start gap-3">
            ...
            <label htmlFor="terms" className="text-sm text-muted-foreground leading-relaxed">
              I agree to the{' '}
              <span className="text-foreground font-medium underline underline-offset-2 cursor-pointer hover:text-primary transition-colors">
                Terms of Service
              </span>{' '}
              and{' '}
              <span className="text-foreground font-medium underline underline-offset-2 cursor-pointer hover:text-primary transition-colors">
        ...
        <motion.p
          ...
          className="text-center mt-8 text-base text-muted-foreground"
        >
